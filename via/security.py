...
RESPONSE_HEADER_BLACKLIST = ["Set-Cookie"]
...
class RequestHeaderSanitiser(object):
    ...
    def __init__(self, application, blacklist=None):
        ...
        self._blacklist = set(
            ["HTTP_" + x.upper().replace("-", "_") for x in blacklist]
        )

    def __call__(self, environ, start_response):
        environ = {k: v for k, v in environ.iteritems() if k not in self._blacklist}
...
class ResponseHeaderSanitiser(object):
    ...
    def __call__(self, environ, start_response):
        def new_start_response(status, response_headers, exc_info=None):
            response_headers = [
                (k, v) for k, v in response_headers if k.lower() not in self._blacklist
            ]
